...
{
public:
    MOCK_METHOD(espnow::AckStatus, handle_payload, (const espnow::AppMessage& msg), (override));
};
...
{
    hub::MessageDispatcher dispatcher(dummy_queue_, mock_espnow_, mock_freertos_);
...
{
    hub::MessageDispatcher dispatcher(nullptr, mock_espnow_, mock_freertos_);
...
TEST_F(MessageDispatcherTest, DispatchLoop_RoutesMessageToRegisteredHandlerAndConfirmsAck)
{
    hub::MessageDispatcher dispatcher(dummy_queue_, mock_espnow_, mock_freertos_);
    MockPayloadHandler mock_handler;
    ...
    test_msg.sequence_number = 42;
    test_msg.requires_ack = true;
    ...
    EXPECT_CALL(mock_espnow_, confirm_reception(test_msg.sender_id, test_msg.sequence_number, espnow::AckStatus::OK))
